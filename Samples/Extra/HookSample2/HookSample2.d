...
__gshared HHOOK keyHook;

int myWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow)
{
    //~ keyHook = enforce(SetWindowsHookEx(WH_KEYBOARD,
                      //~ &KeyboardProc,
    ...
    keyHook = enforce(SetWindowsHookEx(WH_KEYBOARD_LL,
                      &LowLevelKeyboardProc,
                      cast(HINSTANCE)null,    // DLL handle, must be null for current thread handlers
                      0));  // Thread ID, or 0 if used for all desktop threads. Must be 0 for LowLevel hook.
